...
raise RuntimeError('ARCHIVED_DERIVATIVE_NOT_EXECUTABLE')
from pathlib import Path
import json, hashlib, ast, tokenize, tarfile, subprocess, re
ROOT=Path('REPO_ROOT')
OUT=ROOT/'reports/zhinianboke-pdd-review-2026-09-20'
CACHE=ROOT/'.tmp/zhinianboke-pdd-review-2026-09-20'
SRC=CACHE/'source'
SHA='61dac37fb6c329ca397c8bb03d274421254b1498'
evidence={
'S01':('README.md',19,94),
'S02':('websocket/channel_pdd/pdd_channel.py',340,411),
'S03':('websocket/channel_pdd/pdd_channel.py',413,551),
'S04':('websocket/channel_pdd/message_queue.py',34,147),
'S05':('websocket/engine/message_consumer.py',355,541),
'S06':('websocket/engine/message_consumer.py',941,976),
'S07':('websocket/engine/message_consumer.py',1330,1391),
'S08':('common/models/log_models.py',30,80),
'S09':('websocket/channel_pdd/api/get_chat_history.py',103,258),
'S10':('websocket/channel_pdd/api/get_chat_history.py',260,346),
'S11':('websocket/routes/messages.py',35,190),
'S12':('websocket/_bootstrap.py',177,195),
'S13':('websocket/routes/__init__.py',25,45),
'S14':('docker-compose.yml',124,186),
'S15':('websocket/engine/reply_engine.py',306,437),
'S16':('websocket/agent/ai_reply_engine.py',98,184),
'S17':('common/services/kb_service.py',178,225),
'S18':('frontend/src/pages/online_chat.vue',108,163),
'S19':('websocket/channel_pdd/connection_manager.py',38,185),
'S20':('websocket/engine/message_consumer.py',757,840),
'S21':('websocket/channel_pdd/core/base_request.py',238,320),
'S22':('websocket/channel_pdd/api/send_message.py',78,118),
'S23':('websocket/tests/test_pdd_channel.py',1,59),
'S24':('common/core/config.py',90,130),
'S25':('websocket/agent/agent_config.py',29,66),
'S26':('websocket/engine/message_consumer.py',546,597),
'S27':('LICENSE',1,20),
'S28':('websocket/channel_pdd/api/get_conversations.py',35,112),
'S29':('websocket/channel_pdd/api/get_token.py',30,47),
'S30':('websocket/channel_pdd/pdd_channel.py',231,322),
'S31':('websocket/channel_pdd/transfer_service.py',90,128),
'S32':('websocket/channel_pdd/connection_registry.py',65,81),
}
findings=[
('F01','P0','内部执行接口缺少鉴权且默认映射端口',['S11','S12','S13','S14'],'默认源码路由没有服务间鉴权；若网络可达，存在越过 backend 权限层调用店铺执行能力的风险。未扫描任何公网实例。'),
('F02','P1','心跳未等待真正的 Pong',['S03','S23'],'代码只等待 ping 协程返回，其返回的 Pong future 被忽略；心跳正常日志不能证明收到 Pong。对照 websockets 12.0/15.0.1 官方源码确认。'),
('F03','P1','内存队列满时丢消息，非持久消费',['S03','S04','S06'],'队列容量默认 1000，满时拒绝且上层吞掉异常；崩溃和存储失败缺少耐久恢复保障。'),
('F04','P1','实时消息缺少幂等处理',['S05','S07','S08','S10'],'实时落库直接 create，msg_id 非联合唯一键；历史去重不等于实时回复去重。重复推送可能重复记录、计未读和回复。'),
('F05','P1','历史补拉无明确完整性结果',['S09','S10'],'后续页失败、页数达到上限和持久化失败可能仍返回 success=True；不能解释为全历史同步成功。'),
('F06','P1','历史同步阻塞长连接服务事件循环',['S09','S11','S18'],'async 路由直接执行同步 requests/分页 sleep，前端选中会话会触发；可能拖延同进程收消息和心跳。'),
('F07','P1','缺少自动断线补漏闭环',['S02','S09','S18','S19','S30'],'当前调用链有 UI/API 历史同步，但未将其接成连接恢复后的耐久 checkpoint/replay 流程。'),
('F08','P1','转人工暂停状态未闭合',['S05','S20','S31'],'当前消息转人工后返回；未见消费路径持久化并检查会话级暂停态。若后续消息继续推送，可能再次自动回复。'),
('F09','P1','发送超时重试缺少已送达不确定态',['S20','S21','S22'],'发送 POST 继承通用重试；源码没有证明 request_id 服务端幂等，也未见送达回查，故重复发送风险仍未排除。'),
('F10','P2','店铺级单消费者导致跨客户队头阻塞',['S19','S25','S26','S30'],'一家店同一队列逐条等待 AI，慢客户会阻塞同店其它客户；默认整体 AI 预算为 60*(5+1)=360 秒，并非总是耗时六分钟。'),
('F11','P2','默认日志和密钥部署边界需加固',['S03','S24'],'原始聊天报文日志默认开启；配置存在占位 JWT/数据加密/服务间密钥，需生产启动拒绝弱配置及脱敏策略。'),
('F12','P2','人工消息展示与耐久历史不一致',['S05'],'mall_cs 分支只做实时转发，不在此落库；AI 本地上下文不一定即时包含其它端人工回复，不能断言永远无法历史补回。'),
]
text='''# zhinianboke/pdd-auto-reply 独立源码分析
...
上游心跳协议证据：
- @@UPSTREAM15@@：658–680，ping 返回 Pong future。
- @@UPSTREAM12@@：808–839，相同的两阶段等待语义。
...
'''
rows=[];sources=[]
for k,(rel,start,end) in evidence.items():
 p=SRC/rel
 assert p.exists(),p
 assert len(p.read_text(encoding='utf-8-sig').splitlines())>=end,(rel,end)
 rows.append(f'- **{k}**：[{rel}]({p})，L{start}–L{end}。')
 sources.append({'id':k,'path':str(p),'line_start':start,'line_end':end,'url':f'https://github.com/zhinianboke/pdd-auto-reply/blob/{SHA}/{rel}#L{start}-L{end}','sha256':hashlib.sha256(p.read_bytes()).hexdigest()})
text=text.replace('@@EVIDENCE@@','\n'.join(rows)).replace('@@UPSTREAM15@@',str(CACHE/'websockets-15-connection.py')).replace('@@UPSTREAM12@@',str(CACHE/'websockets-12-protocol.py'))
(OUT/'analysis.md').write_text(text)
(OUT/'sources.json').write_text(json.dumps({'repository':'https://github.com/zhinianboke/pdd-auto-reply','commit':SHA,'review_date':'2026-09-20','sources':sources,'upstream_urls':['https://raw.githubusercontent.com/python-websockets/websockets/15.0.1/src/websockets/asyncio/connection.py','https://raw.githubusercontent.com/python-websockets/websockets/12.0/src/websockets/legacy/protocol.py']},ensure_ascii=False,indent=2))
manifest={};mismatches=[]
with tarfile.open(CACHE/'source.tar.gz','r:gz') as t:
 for m in t.getmembers():
  if not m.isfile():continue
  rel='/'.join(Path(m.name).parts[1:]);b=t.extractfile(m).read();p=SRC/rel
  manifest[rel]=hashlib.sha256(b).hexdigest()
  if not p.exists() or p.read_bytes()!=b:mismatches.append(rel)
assert not mismatches,mismatches
inventory=json.loads((CACHE/'static-inventory.json').read_text())
assert not inventory['syntax_errors']
tracked=subprocess.run(['git','diff','--exit-code'],cwd=ROOT,capture_output=True).returncode
staged=subprocess.run(['git','diff','--cached','--exit-code'],cwd=ROOT,capture_output=True).returncode
state=json.loads((ROOT/'project/PROJECT_STATE.json').read_text())
report={'result':'COMPLETE','scope':'INDEPENDENT_PUBLIC_SOURCE_STATIC_ANALYSIS_ONLY','date':'2026-09-20','commit':SHA,'archive_sha256':hashlib.sha256((CACHE/'source.tar.gz').read_bytes()).hexdigest(),'third_party_execution':'NOT_RUN','pytest':'NOT_RUN','build':'NOT_RUN','integration':'NOT_RUN','live_validation':'NOT_RUN','next_stage_not_executed':True,'findings':[{'id':i,'priority':p,'classification':'CONFIRMED','title':t,'evidence_ids':e,'interpretation':c,'consequences':'INFERRED; no live reproduction'} for i,p,t,e,c in findings],'static_validation':{'archive_files':len(manifest),'source_mismatches':mismatches,'python_parsed':inventory['python_files'],'syntax_errors':inventory['syntax_errors'],'test_files':len(inventory['test_files']),'test_definitions':inventory['test_functions'],'tracked_diff_empty':tracked==0,'staged_diff_empty':staged==0},'authorization_unchanged':{'current_task':state.get('current_task'),'current_execution_authorization':state.get('current_execution_authorization'),'current_live_evidence_authorization':state.get('current_live_evidence_authorization')},'artifacts':[str(OUT/n) for n in ['analysis.md','sources.json','report.json','source-manifest.json','build_report.py']]}
(OUT/'source-manifest.json').write_text(json.dumps(manifest,indent=2))
(OUT/'report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
assert tracked==0 and staged==0
for f in report['artifacts']:assert Path(f).exists(),f
print(json.dumps({'result':report['result'],'report':str(OUT/'analysis.md'),'validation':report['static_validation'],'characters':len(text)},ensure_ascii=False,indent=2))
