// M4 prompt golden executor (TASK-019): derives fixtures from disk and genuinely
...
const FIXTURES = join(HERE, "..", "..", "parity-tests", "fixtures", "prompt");
...
console.log("Discovered " + files.length + " fixtures from disk (prompt).");
...
const code = [
  ...
  "r = run_golden_group(" + "\"prompt\"" + ", " + JSON.stringify(FIXTURES) + ")",
  "print(json.dumps(r))",
].join("\n");
...
const cases = result.results.map((r) => {
  ...
  return {
    ...
    implemented_test: "verify-m4-prompt-goldens.mjs -> m4_helpers (real subsystem)",
...
writeFileSync(join(REBUILD, "reports", "m4-prompt-fixture-results.json"), JSON.stringify({ schema_version: "1.0", cases, all_passed: result.failed === 0 }, null, 2) + "\n", "utf-8");
