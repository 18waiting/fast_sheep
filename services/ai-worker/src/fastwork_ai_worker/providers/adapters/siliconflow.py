"""SiliconFlow chat/completions adapter (TASK-019 M4).

OpenAI-style generation with a SiliconFlow base URL default.  Pure data; no
...
def build_request(
    ...
    """Build a data-only SiliconFlow request envelope."""
    base_url = str(pick_config(provider_config, "base_url", "api_url", default=DEFAULT_SILICONFLOW_BASE_URL))
...
def parse_response(raw: Dict[str, Any], request_id: str) -> Dict[str, Any]:
    ...
    if not isinstance(raw, dict):
        raise invalid_response("siliconflow response must be an object")
    choices = raw.get("choices") or []
    if not choices or not isinstance(choices, list):
        raise invalid_response("siliconflow response has no choices")
