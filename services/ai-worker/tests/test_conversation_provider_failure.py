...
class TestProviderFailure(unittest.TestCase):
    def test_provider_error_normalized(self):
        e = make_engine(generation_provider=FakeGenerationProvider(error={"category": "provider", "retryable": True}))
        r = e.generate({"question": "测试"})
        self.assertEqual(r["error"]["category"], "provider")
        self.assertTrue(r["error"]["retryable"])
        decs = [t.get("decision") for t in r["trace"] if t["operation"] == "MODEL_ROUTE"]
        self.assertIn("fallback", decs)
