...
class TestLEARNGoldens(unittest.TestCase):
    def test_all_discovered(self):
        ...
        for f in discover():
            with open(f, encoding="utf-8") as fh:
                fx = json.load(fh)
            if not fx.get("case_id", "").startswith("GF-LEARN"):
                continue
            result = run_learning_case(fx)
            results.append({"case_id": fx["case_id"], "result": result})
            if result != "PASS":
                failed += 1
        self.assertEqual(failed, 0, str([x for x in results if x["result"] != "PASS"]))
        self.assertEqual(len(results), 11, "discovered GF-LEARN fixtures")
