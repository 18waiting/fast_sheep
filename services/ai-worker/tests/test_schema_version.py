...
class TestSchemaVersion(unittest.TestCase):
    def test_supported(self) -> None:
        self.assertEqual(SUPPORTED_DB_SCHEMA_VERSION(), 7)
        self.assertTrue(is_schema_supported(7))
        self.assertFalse(is_schema_supported(6))

    def test_worker_schema_version_reads_app_meta(self) -> None:
        tmp, conn = make_migrated_db()
        self.assertEqual(worker_schema_version(conn), 7)
        conn.close()
