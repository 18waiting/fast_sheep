...
> **历史证据快照（非当前 gap 状态）**：下文 `OPEN / NOT_EVIDENCED` 记录 SHEEP-032 当时的参考范围结论。其后 Owner 在 [REFERENCE_GAP_RESOLUTION_CHECKPOINT](reference-gap-resolution-checkpoint.md) 将 G2 处置为 `CLEAN_ROOM`，并未授权扩大 reference discovery。本文原始证据保留；当前产品方向与执行授权分别以 [North Star](product/FAST_SHEEP_NORTH_STAR.md) 和 [PROJECT_STATE.json](../project/PROJECT_STATE.json) 为准。
