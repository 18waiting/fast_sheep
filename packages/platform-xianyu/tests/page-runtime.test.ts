...
import { XianyuPageRuntime, dispatchXianyuCommand, XIANYU_COMMAND_ALLOWLIST, FORBIDDEN_COMMAND_TYPES } from "../dist/index.js";
import { loadFixture } from "./helpers.ts";
import { xianyuSendText, xianyuExecuteTransfer } from "../dist/index.js";

test("page runtime boots, scans inbound, and routes commands", async () => {
  const events: string[] = [];
  const doc = loadFixture("chat-basic.html");
  const runtime = new XianyuPageRuntime({
    platform: "xianyu",
    doc,
    sessionId: "s1",
    shopId: "shop-1",
    profile: { version: "xianyu-dom-1.0.0", platform: "xianyu", entries: [] },
    handlers: {
      health: (c) => ({ command_id: c.command_id, ok: true }),
      scan: (c) => ({ command_id: c.command_id, ok: true, result: { messages: [] } }),
      send_text: (c) => { const r = xianyuSendText(doc, c.conversation_id ?? "", c.text ?? ""); return { command_id: c.command_id, ok: r.ok, result: { message_id: r.message_id } }; },
      send_image: (c) => ({ command_id: c.command_id, ok: false, error: "capability.unsupported" }),
      transfer: (c) => { const r = xianyuExecuteTransfer(doc, c.target ?? ""); return { command_id: c.command_id, ok: r.ok, result: { executed: r.executed } }; },
...
test("command dispatch is a finite allowlist; forbidden types rejected", () => {
  assert.ok(XIANYU_COMMAND_ALLOWLIST.includes("send_text"));
  for (const t of FORBIDDEN_COMMAND_TYPES) {
    assert.ok(!XIANYU_COMMAND_ALLOWLIST.includes(t as never));
  }
  const r = dispatchXianyuCommand({ type: "execute_js", command_id: "x", session_id: "s" }, {
