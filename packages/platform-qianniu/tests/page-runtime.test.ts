...
import { QianniuPageRuntime, dispatchQianniuCommand, QIANNIU_COMMAND_ALLOWLIST, FORBIDDEN_COMMAND_TYPES } from "../dist/index.js";
import { loadFixture } from "./helpers.ts";
import { qianniuSendText, qianniuExecuteTransfer } from "../dist/index.js";

test("page runtime boots, scans inbound, and routes commands", async () => {
  const events: string[] = [];
  const doc = loadFixture("chat-basic.html");
  const runtime = new QianniuPageRuntime({
    platform: "qianniu",
    doc,
    sessionId: "s1",
    shopId: "shop-1",
    profile: { version: "qianniu-dom-1.0.0", platform: "qianniu", entries: [] },
    handlers: {
      health: (c) => ({ command_id: c.command_id, ok: true }),
      scan: (c) => ({ command_id: c.command_id, ok: true, result: { messages: [] } }),
      send_text: (c) => { const r = qianniuSendText(doc, c.conversation_id ?? "", c.text ?? ""); return { command_id: c.command_id, ok: r.ok, result: { message_id: r.message_id } }; },
      send_image: (c) => ({ command_id: c.command_id, ok: false, error: "capability.unsupported" }),
      transfer: (c) => { const r = qianniuExecuteTransfer(doc, c.target ?? ""); return { command_id: c.command_id, ok: r.ok, result: { executed: r.executed } }; },
...
test("command dispatch is a finite allowlist; forbidden types rejected", () => {
  assert.ok(QIANNIU_COMMAND_ALLOWLIST.includes("send_text"));
  for (const t of FORBIDDEN_COMMAND_TYPES) {
    assert.ok(!QIANNIU_COMMAND_ALLOWLIST.includes(t as never));
  }
  const r = dispatchQianniuCommand({ type: "execute_js", command_id: "x", session_id: "s" }, {
