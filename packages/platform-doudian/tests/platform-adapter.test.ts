...
import { createDoudianPlatformAdapter } from "../dist/index.js";
import { DoudianSessionState } from "../dist/index.js";
import type { DoudianPageBridge } from "../dist/index.js";

function makeAdapter() {
  const session = new DoudianSessionState("doudian", "shop-1", "s1");
  session.setStatus("READY");
  const calls: string[] = [];
  const bridge: DoudianPageBridge = {
    execute: async (cmd) => { calls.push(cmd.type + ":" + (cmd.text ?? cmd.target ?? "")); return { command_id: cmd.command_id, ok: true, result: { message_id: "ack" } }; },
  };
  const adapter = createDoudianPlatformAdapter({ bridge, session, commandIdFactory: () => "c1" });
