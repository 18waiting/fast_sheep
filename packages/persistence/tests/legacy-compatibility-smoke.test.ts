...
// - Core migration expectation: legacy 0001-0004 -> schema v7 (4 -> 7), with
...
test("compat: legacy v4 DB + synthetic data -> v7; data preserved; checksums unchanged; reopen smoke", () => {
  const r = root();
  ...
  assert.equal(res.migratedCount, 3, "0005+0006+0007 applied");
  assert.equal(res.backedUp, true);
  assert.equal(conn.get("SELECT COUNT(*) AS c FROM schema_migrations").c, 7, "end: 7 migrations (0001-0007)");
  const appliedVersions = runner.applied(conn).map((a) => a.version).sort((x, y) => x - y);
  assert.deepEqual(appliedVersions, [1, 2, 3, 4, 5, 6, 7], "intermediate versions verifiable");
...
test("compat: fresh v7 — legacy + domain coexist; same text ID collision is isolated per table", () => {
  const r = root();
