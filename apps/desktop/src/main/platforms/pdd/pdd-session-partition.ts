// M7 per-shop Chromium partition (clean-room). Logical partition: persist:shop-<id>.
// Safe characters only; stable deterministic mapping; no path traversal; no secrets.
export function sanitizeShopId(shopId: string): string {
  return shopId.replace(/[^A-Za-z0-9._-]/g, "_");
}

export function partitionFor(shopId: string): string {
