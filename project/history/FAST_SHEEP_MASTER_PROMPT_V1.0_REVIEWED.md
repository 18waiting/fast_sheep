# FAST_SHEEP_MASTER_PROMPT_V1.0_REVIEWED.md

# 快羊客服（Fast Sheep）项目母提示词 / Project Constitution

> **Document Version**：1.0 Reviewed Candidate  
> **Review Status**：REVIEW-1 REPAIRS APPLIED — Governance Dry Run 001 PASS; pending Owner Review  
> **Governance Repair**：GOV-REPAIR-001 + GOV-REPAIR-002 applied after Dry Run 001  
...
# 42. Review-1 Owner Review Points

本 Reviewed Candidate 在 Review 1 中新增/强化的关键商业原则如下，需由 Release Owner 最终确认：

1. **UI / Asset License**：Track A 可以进行内部参考还原，但任何许可/再分发状态未知的原始或第三方资产不得默认进入商业 release；应 review、替换或重设计。
2. **Merchant Data Ownership**：商户业务数据按明确治理策略支持导出、删除、保留与恢复；授权到期不得成为“扣留用户已有数据”的手段。
3. **Managed AI / BYOK Data Disclosure**：只发送完成任务必要的最小上下文，Secret 默认永不进入模型上下文；Managed AI 与 BYOK 的数据流向必须透明。
4. **Platform Compliance**：技术接入成功不等于允许商用；PDD / DouDian 等进入 `COMMERCIAL_SUPPORTED` 前必须通过平台合规 Gate。
5. **Update / Migration Safety**：不允许 destructive silent migration；升级必须有兼容、备份与恢复验证。
6. **Backup / Recovery**：Local-first 关键业务数据必须具备经过实际测试的备份与恢复路径。

当前治理顺序已更新为：

- `Governance Dry Run 001` 已完成并由 Controller 判定 `PASS`；
- 当前仅剩 Owner 对 Review-1 六项 owner points 的明确确认；
- Owner 确认后，Controller 执行 Master Prompt V1.0 approval/freeze；
- Master 状态正式变为 `APPROVED V1.0` 后，才进入 Review 2 / Review 3 的最终治理收口；
- 在最终 governance freeze 完成前，不启动正式 `SHEEP-001` coding。
