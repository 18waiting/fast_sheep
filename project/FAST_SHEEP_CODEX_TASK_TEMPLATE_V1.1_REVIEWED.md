# FAST_SHEEP_CODEX_TASK_TEMPLATE_V1.1_REVIEWED.md

# 快羊客服 Codex 通用任务模板 — Reviewed V1.1
...
> **Status**：REVIEWED — CURRENT V1.1 AUTHORITY — SHEEP-300 NOT_AUTHORIZED
> **Review History**：
> - 2026-08-22：Task Template V1.0 初版
> - 2026-09-17：Roadmap V1.1 migration adds mandatory product, identity, side-effect, UNKNOWN, target-proof, retry, verification, and audit preflight fields.
> **Supersedes**：`FAST_SHEEP_CODEX_TASK_TEMPLATE_V1.0_REVIEWED.md` as the current task template authority; V1.0 remains immutable historical governance.
...
【Review 3 修订】当前状态（2026-08-23）：

- active Master 已 APPROVED V1.0：`E:\fast_sheep\project\FAST_SHEEP_MASTER_PROMPT.md`
  （exact path 一律以 PROJECT_STATE `master_prompt.path` 为准，不得凭文件名猜测）
- `E:\fast_sheep\project\history\FAST_SHEEP_MASTER_PROMPT_V1.0_REVIEWED.md` 是历史 reviewed 候选，
  仅作 provenance，**永远不作为 active Master**

然后读取：

`E:\fast_sheep\project\DECISIONS.md`

然后读取当前 active Coding Roadmap：

- Freeze 后以 `E:\fast_sheep\project\FAST_SHEEP_CODING_ROADMAP_V1.1_REVIEWED.md` 为准
  （exact path 以 PROJECT_STATE `roadmap.path` / `roadmap.reviewed_path` 为准）

【Review 3 修订】Review 2 契约意识（R-06 / R-07）：

- 涉及 domain persistence / entity identity / sync-readiness 的任务必须遵守 Phase 1
  **Sync-Ready Persistence Contract**（stable identity、change metadata 能力、revision 能力、
  mutation boundary、authority hook；`sync-ready ≠ sync-enabled`；能力级，不锁字段名）
- 除 Phase 12 任务外，禁止实现 Outbox / sync state / sync 传输
- 涉及 Cloud sync scope / retention / deletion / backup 的行为，必须先过 R-06
  `DATA-DECISION-GATE`（未关闭前属 Owner DATA 决策，不得自行实现）

不得因旧模板示例或历史文件名而自行创建/改名 Master Prompt 或 Roadmap。
...
- R-06 相关（Cloud sync scope / retention / deletion / backup）未获 Owner DATA 决策前，
  相关行为必须保持 DEFERRED / PRODUCT_DECISION_REQUIRED，不得以实现便利为由提前决定。
---
...
【Review 3 修订】涉及 domain persistence / sync-readiness 的任务，exact inputs 应包含：

```text
E:\fast_sheep\project\FAST_SHEEP_CODING_ROADMAP_V1.0_REVIEWED.md   （对应 Phase / Milestone / Gate）
```
...
11. 是否修改/消费 Phase 1 Sync-Ready Persistence Contract（R-07：stable identity /
    change metadata 能力 / revision 能力 / mutation boundary / authority hook；
    能力级，不锁字段名）。
12. 是否触发 R-06 `DATA-DECISION-GATE`（Cloud sync scope / retention / deletion / backup）。
13. 是否触碰 Phase 12 sync/Outbox 实现边界（除 Phase 12 任务外，禁止实现）。

---

# 6A. MANDATORY PRE-IMPLEMENTATION PRODUCT AND SAFETY BLOCK
...
For any task involving a platform side effect, Codex must also produce:
...
- Product/architecture authority is read from the exact paths in
  `PROJECT_STATE.json`.
- Identity semantics reference `docs/architecture/AI_CUSTOMER_SERVICE_CORE.md`.
- Adapter/result semantics reference
  `docs/architecture/PLATFORM_ADAPTER_CONTRACT.md`.
- Execution authorization and UNKNOWN/retry behavior reference
  `docs/architecture/REPLY_AND_ACTION_SAFETY.md`.
- `SHADOW` must prove `TRANSPORT SEND CALLS = 0`.
- No task may grant production AUTO authorization implicitly.
...
【Review 3 修订 / R-03】Entitlement 本地 enforcement：没有有效 authoritative / signed /
permitted evidence 时不得 fail-open；`UNKNOWN => DISABLED` 不得作为永久写死规则。

## DATA
Cloud 同步内容、删除、保留、隐私、迁移。

【Review 3 修订 / R-06】Cloud sync scope / retention / deletion / backup 属 Owner DATA
决策；`DATA-DECISION-GATE` 未关闭前，不实现对应 sync/删除行为。
...
- 未获授权的 sync / Outbox 实现（Phase 12 之外）
- 未获 R-06 `DATA-DECISION-GATE` 的 sync scope / retention / deletion 行为
- 治理文件（Master / DECISIONS.md / PROJECT_STATE.json / Roadmap / Task Template / AGENTS.md），
  除非当前任务明确授权修改
---
...
【Review 3 修订 / R-07】通用 Sync-readiness：

```text
sync-ready != sync-enabled
Phase 1 Sync-Ready Persistence Contract 提供能力预留
（stable identity / change metadata / revision / mutation boundary / authority hook）
Outbox 与 Sync Engine 只在 Phase 12 实现
具体实体是否 sync 由 R-06 DATA-DECISION-GATE 决定
```
...
- no unapproved sync/Outbox implementation（Phase 12 之外）
- no sync scope/retention/deletion behavior without R-06 DATA-DECISION-GATE
...
【Review 3 修订 / R-07】额外记录：

```text
unapproved_sync_or_outbox_added = true/false
```
...
如果 `unapproved_sync_or_outbox_added = true` 且任务不是 Phase 12 sync 任务：
...
【Review 3 修订 / R-06 / R-07】如果任务影响 domain persistence / schema：

- 必须与 Phase 1 Sync-Ready Persistence Contract 一致（能力级：stable identity /
  change metadata / revision / mutation boundary；不锁字段名、不强制所有表有 sync revision）
- 若任务会让某实体实际 sync，R-06 `DATA-DECISION-GATE` 必须先 PASS；
  未 PASS 前不得实现 sync 行为
- 若触发 R-06 范围（retention / deletion / backup），未获 Owner DATA 决策前不得实现对应行为
...
- UI asset provenance 必须记录（SHEEP-024 / GATE-P2）
- 未授权 redistributable asset 不进入 commercial asset set
- IA / ideas / independently redesigned output 不与“未授权 asset”混淆
---
...
【Review 3 修订】建议 task-specific fields（可选）：

```json
{
  "sync_readiness_contract_consumed": false,
  "data_decision_gate_triggered": false
}
```
...
14. 未在 Phase 12 之外实现 sync / Outbox（或未获 R-06 决策的 sync 行为）。
15. 未未经授权修改治理文件（Master / DECISIONS.md / PROJECT_STATE.json / Roadmap / Task Template / AGENTS.md）。
...
```text
Sync/Outbox Added Outside Phase 12:
NO
```
...
- Task Template V1.1 is the current task-template authority when resolved
  through `PROJECT_STATE.json`.
- The V1.1 preflight block adds product alignment, canonical authority read,
  identity scope, side-effect class, UNKNOWN behavior, stop condition, and
  platform-side-effect target/retry/verification/audit fields.
- `SHEEP-300` remains `NOT_STARTED / NOT_AUTHORIZED`.
- AUTO production authorization remains separate and is never implied by task
  completion.
